...
import { usePublicEquipment, usePublicEquipmentCategories } from "../../hooks/queries/useEquipment";
...
export default function FleetInventory() {
  ...
  const [selectedItem, setSelectedItem] = useState<PublicEquipment | null>(null);
  ...
  const isLoading = isEquipmentLoading || isCategoriesLoading;
  const isError = isEquipmentError || isCategoriesError;
  const isRetrying = isEquipmentFetching || isCategoriesFetching;
  ...
  const filteredFleet = useMemo(() => {
    ...
    return equipment.filter((item) => {
      const matchesTab = activeTab === "all" || item.category.slug === activeTab;
      ...
      const matchesSearch =
        ...
        item.primarySpecification.label.toLowerCase().includes(searchValue) ||
        item.primarySpecification.value.toLowerCase().includes(searchValue);
  ...
  }, [filteredFleet, safeCurrentPage, itemsPerPage]);
  ...
  const handleRetry = () => {
    void Promise.all([refetchEquipment(), refetchCategories()]);
  };
  ...
  return (
    <section id="fleet-inventory">
        ...
        <div className="mt-12 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="relative w-full lg:w-[320px]">
            ...
            <input
              ...
              className="h-12 w-full rounded-xl border border-transparent bg-blue-01/5 pl-12 pr-4 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-01/30 focus:bg-white disabled:cursor-not-allowed disabled:opacity-60"
            />
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <>
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {paginatedFleet.map((item, index) => (
              <FleetCard
                key={item._id}
                item={item}
                animationDelay={(index % 3) * 60}

...
const getJobRequestErrorMessage = (error: unknown): string => {
  ...
  if (status === 409) {
    if (
      ...
      !backendMessage.toLowerCase().includes("route")
    ) {
      return backendMessage;
    }
...
export default function JobDetailsModal({
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !createJobRequest.isPending) {
        handleClose();
      }
    };
  ...
  const isOpen =
    job.status === "published" && new Date(job.deadline) >= new Date();
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_CV_SIZE) {
      ...
      setFormErrors((previous) => ({
        ...previous,
        cv: "CV file size cannot exceed 10 MB.",
      }));
      event.target.value = "";
      return;
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setApiError(getJobRequestErrorMessage(error));
    }
  };

  const isSubmitted =
    createJobRequest.isSuccess && createJobRequest.data?.success === true;
  ...
  return (
    ...
    >
      <div className="relative max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
        {/* Fixed close button inside scroll container */}
        <div className="sticky top-0 z-50 h-0 w-full">
          <button
            ...
            className="absolute right-5 top-5 cursor-pointer rounded-full bg-slate-100 p-2 text-slate-500 transition hover:bg-red-100 hover:text-red-01 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {isSubmitted && (
          <div className="p-6 md:p-8">
            <div className="flex flex-col items-center justify-center rounded-2xl bg-[#f7f8ff] px-6 py-12 text-center">
              ...
              <button
                type="button"
                onClick={handleClose}
                className="mt-8 h-12 cursor-pointer rounded-xl bg-blue-01 px-8 text-sm font-bold text-white transition hover:bg-red-01"
              >
                Close
              </button>
            </div>
          ...
          <div className="p-4 sm:p-6 md:p-8">
            ...
            <div className="mt-8 flex flex-col gap-3 border-t border-slate-100 pt-6 md:flex-row md:justify-end">
              <button
                type="button"
                onClick={handleClose}
                className="h-12 cursor-pointer rounded-xl border border-slate-200 px-8 text-sm font-bold text-slate-600 transition hover:bg-slate-100"
              >
                Cancel
              </button>
              ...
              <button
                ...
                className="inline-flex h-12 cursor-pointer items-center justify-center gap-2 rounded-xl bg-blue-01 px-8 text-sm font-bold text-white transition hover:bg-red-01 disabled:cursor-not-allowed disabled:opacity-50"
              >
        ...
        {!isSubmitted && step === 2 && (
          <form noValidate onSubmit={handleSubmit} className="p-6 md:p-8">
            <p className="mb-6 text-sm leading-6 text-slate-500">
              Please fill in your information and upload your CV to apply for this
              position.
            </p>
            ...
            <div className="grid gap-x-4 gap-y-5 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <span className="mb-2 block text-sm font-semibold text-blue-01">
                  Upload CV
                  <span className="ml-1 text-red-01">*</span>
                </span>

                <label
                  htmlFor="cv"
                  className={`flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed p-6 text-center transition ${formErrors.cv
                ...
                >
                  <Upload
                    className={`mb-3 h-8 w-8 ${formErrors.cv ? "text-red-01" : "text-blue-01"
                      }`}
                  />
            ...
            <div className="mt-8 flex flex-col gap-3 border-t border-slate-100 pt-6 md:flex-row md:justify-between">
              <button
                ...
                disabled={createJobRequest.isPending}
                className="inline-flex h-12 cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-200 px-8 text-sm font-bold text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <div className="flex flex-col gap-3 md:flex-row">
                <button
                  ...
                  onClick={handleClose}
                  disabled={createJobRequest.isPending}
                  className="h-12 cursor-pointer rounded-xl border border-slate-200 px-8 text-sm font-bold text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Cancel
                </button>
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`h-12 w-full rounded-xl border px-4 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-blue-01 ${error

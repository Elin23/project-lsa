...
export default function ContactInfo() {
  const { data: contact, isLoading, isError, isFetching, refetch } = useContactInfo();
  ...
  if (isError) {
    return (
      <section aria-labelledby="contact-info-heading" data-aos="fade-up" data-aos-duration="800" data-aos-once="true">
        <SectionState
          variant="error"
  ...
  const hasAddress = Boolean(contact.location?.address?.trim());

  const phones = Array.isArray(contact.phones)
    ? contact.phones.filter((phone) => Boolean(phone?.trim()))
    : [];
  ...
  const hasWorkingHours = Boolean(contact.workingHours?.trim());

  const hasEmergencyHours = Boolean(contact.emergencyHours?.trim());
  ...
  return (
    <section aria-labelledby="contact-info-heading" data-aos="fade-up" data-aos-duration="800" data-aos-once="true">
      <h2 id="contact-info-heading" className="text-[30px] font-bold leading-tight text-blue-01 md:text-[38px]">
        {contact.title}
      </h2>
      ...
      <address className="not-italic">
        <ul className="mt-9 space-y-7">
          ...
          {hasAddress && (
            <li className="group flex gap-4 transition-transform duration-300 ease-out hover:translate-x-1">
              ...
              <div>
                ...
                <div className="mt-1 whitespace-pre-line text-sm leading-5 text-muted-blue">
          ...
          {/* Working Hours */}
          {(hasWorkingHours || hasEmergencyHours) && (
            <li className="group flex gap-4 transition-transform duration-300 ease-out hover:translate-x-1">
              <div
                aria-hidden="true"
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-blue-01/10 text-blue-01 transition-[background-color,color] duration-300 ease-out group-hover:bg-blue-01 group-hover:text-white"
              ...
              <div>
                ...
                <div className="mt-1 text-sm leading-5 text-muted-blue">
                  {hasWorkingHours && <span className="block">{contact.workingHours}</span>}

                  {hasEmergencyHours && <span className="block">{contact.emergencyHours}</span>}
                </div>
              </div>
            </li>
          )}

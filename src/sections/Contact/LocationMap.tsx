...
const getGoogleMapsEmbedUrl = (mapUrl: string): string | null => {
  try {
...
export default function LocationMap() {
  ...
  const mapUrl = contact?.location?.mapUrl?.trim() || "";

  const googleMapsEmbedUrl = useMemo(
    () => (mapUrl ? getGoogleMapsEmbedUrl(mapUrl) : null),
    [mapUrl],
  );
  ...
  if (isError) {
    return (
      <section className="relative left-1/2 w-screen -translate-x-1/2 px-container">
        <SectionState
          variant="error"
  ...
  return (
    <section aria-label="Company location" className="relative left-1/2 w-screen -translate-x-1/2 overflow-hidden">
      <div className="relative h-80 w-full bg-[#ECEFF5] md:h-105 lg:h-130">
        {!loaded && <MapSkeleton />}

        <iframe
          title={contact?.title || "LSA Location"}
...
function MapSkeleton() {
  return (
    <div aria-hidden="true" className="absolute inset-0 z-10 animate-pulse bg-[#ECEFF5]">

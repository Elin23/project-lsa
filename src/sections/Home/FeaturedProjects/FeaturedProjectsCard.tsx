...
export default function FeaturedProjectsCard({ image, category, title, path }: FeaturedProjectsCardProps) {
  return (
    <Link to={path} data-aos="flip-left" className="group relative block overflow-hidden rounded-xl bg-black">
      {/* Image */}
      <img
        src={image}
        alt={title}
      ...
      <div className="absolute right-4 top-3">
        <span className="inline-flex items-center rounded-full border border-white/70 bg-white/95 px-2.5 py-1 text-[8px] font-semibold uppercase tracking-[0.04em] text-slate-600 shadow-sm">
          {category}
        </span>
      </div>
